/*
  ...
  1. New Tables
    - `webhook_events`: Stores PayPal webhook events for audit and idempotency
    - `payment_transactions`: Records all payment activities
    - `notification_queue`: Manages user notifications for payment events

  2. User Settings Updates
    - Added subscription-related columns to user_settings table
    - Added payment tracking fields

  3. Security
    - Enabled RLS on all new tables
    - Created appropriate policies for service role and authenticated users
