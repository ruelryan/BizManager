/*
  # Payment System and User Settings Enhancement
  
  This migration adds payment-related functionality and enhances user settings.
  
  1. New Columns
     - Adds plan, subscription details, and payment status to user_settings
  
  2. New Tables
     - webhook_events: For tracking and processing PayPal webhook events
     - payment_transactions: For recording payment history
     - notification_queue: For managing user notifications
  
  3. Security
     - Enables RLS on all new tables
     - Adds appropriate policies for data access control
...
-- Create updated_at trigger for payment_transactions if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
