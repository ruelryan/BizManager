...
export function Login() {
  ...
  // Track Facebook Click ID when component mounts
  React.useEffect(() => {
    trackFacebookClickId();
  }, []);
      ...
      if (showSignUp && typeof window !== 'undefined' && window.fbq) {
        try {
          window.fbq('track', 'CustomEvent', {
            event_name: 'SignupFailed',
            error_message: errorMessage,
            email_provided: !!formData.email,
            name_provided: !!formData.name
          });
        } catch (fbError) {
          console.error('Failed to track signup failure:', fbError);
        }
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 flex items-center justify-center p-4 transition-colors duration-200">
      ...
      {process.env.NODE_ENV === 'development' && (
        <SignupAnalytics isVisible={true} />
      )}
    </div>
  );
}